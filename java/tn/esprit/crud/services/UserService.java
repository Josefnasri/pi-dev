...
String req="INSERT INTO user(nom , prenom , adresse , email , mdp) VALUES( '"+user.getNom()+"' , '"+user.getPrenom()+"' , '"+user.getAdresse()+"' , '"+user.getEmail()+"' , '"+user.getMdp()+"')";
    ...
    public int getIdbyMail(String EmailUser) throws SQLException {
        PreparedStatement st = connection.prepareStatement("select id from user where email=?");
    ...
    public void UpdatePassword(String EmailUser, String PasswordUser) throws SQLException {
      ...
      //  String sql = "UPDATE user SET mdp='" + cpass + "'" + "WHERE email='" + EmailUser + "'";

        String sql = "UPDATE user SET mdp='" + PasswordUser + "'" + "WHERE email='" + EmailUser + "'";
