...
/*
 ...
 /*   @Override
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
        List<EntreePanier> entries = PanierService.getInstance().getEntries(getUser());
        ...
        if ("Visitor".equalsIgnoreCase(getUser().getUserRole())) {
            if (entries.isEmpty()) {
                ...
                try {
                    ...
                    for (EntreePanier panierEntry : entries) {
                        try {
                            HBox hbox = panierEntryView(panierEntry);
                            Label prodcutName = new Label(panierEntry.getProduct().getNom());
 ...
 /*               } catch (FileNotFoundException ex) {
            ...
            Map<String, List<EntreePanier>> allEntries = PanierService.getInstance().getAllEntries();
            for (Map.Entry<String, List<EntreePanier>> panierEntry : allEntries.entrySet()) {
                try {
                    try {

                        Label UserNamePanierLabel = new Label("Panier de :" + panierEntry.getKey());
                        PanierPane.getChildren().add(UserNamePanierLabel);
                        List<EntreePanier> childMap = panierEntry.getValue();
                        for (EntreePanier entry : childMap) {
                            HBox hbox = panierEntryView(entry);
                            Label prodcutName = new Label(entry.getProduct().getNom());
                            hbox.getChildren().add(prodcutName);
                            PanierPane.getChildren().add(hbox);
                        }
...
/*                } catch (SQLException ex) {
    ...
    private HBox panierEntryView(EntreePanier panierEntry) throws FileNotFoundException {
        HBox layout = new HBox();
        ...
        Label productName = new Label(panierEntry.getProduct().getNom());
        ...
        plusButton.setUserData(panierEntry.getProduct().getNom());
        ...
        moinsButton.setUserData(panierEntry.getProduct().getNom());
        ...
        Label labelPrice = new Label(String.valueOf("£ " + panierEntry.getProduct().getPrix()));
