...
/*    Connection conn;
    ...
    public void addProduct(String productName, User user) {
        EntreePanier productEntry = Panier.getInstance().getEntries().get(productName.toUpperCase());
        if (productEntry != null) {
            productEntry.increaseQuantity();
            updateEntety(productEntry);
        } else {
            try {
                Produit product = new Produit (); //.valueOf(productName); //.getNom();//
                product.setNom();
                Produit product = Produit.valueOf(productName); //.getNom();//
                EntreePanier entry = new EntreePanier(product, 1);
                Panier.getInstance().getEntries().put(productName.toUpperCase(), entry); //product.getName
...
/*
    ...
    public void removeProduct(String prodcutName) {
        EntreePanier panierEntry = Panier.getInstance().getEntries().get(prodcutName.toUpperCase());
    ...
    public int getQuantity(String productName) {
        EntreePanier entry = Panier.getInstance().getEntries().get(productName.toUpperCase());
        if (entry != null) {
    ...
    public float calculTotale() {
        float totale = 0;
        for (EntreePanier entry : Panier.getInstance().getEntries().values()) {
            float entryCost = entry.getProduct().getPrix() * entry.getQuantity();
            totale += entryCost;
        }
        return totale;
    }
    ...
    public float calculTotaleByCategory(String category) {
        ...
        float totale = 0;
        for (EntreePanier entry : Panier.getInstance().getEntries().values()) {
            if (category.equalsIgnoreCase(entry.getProduct().getCategorie())) {
                float entryCost = entry.getProduct().getPrix() * entry.getQuantity();
                totale += entryCost;
            }
        }
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                while (rs.next()) {
                    ...
                    product = Produit(rs.getString("name"));
                    EntreePanier entry = new EntreePanier(product, quantity,date);
    ...
    public Map<String, List<EntreePanier>> getAllEntries() {
        ...
        User user = null;
        int quantity;
        Map<String, List<EntreePanier>> allEntries = new HashMap<String, List<EntreePanier>>();
        Produit product = null;
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                while (rs.next()) {
                    ...
                    product = new Produit(rs.getString("name"));
                    EntreePanier entry = new EntreePanier(product, quantity);
                    Panier.getInstance().getEntries().put(rs.getString("name"), entry);
                }
    ...
    public float calculTotaleByEntries(List<EntreePanier> panierEntries) {
        float totale = 0;
        for (EntreePanier entry : panierEntries) {
            float entryCost = entry.getProduct().getPrix() * entry.getQuantity();
            totale += entryCost;
        }
        return totale;
    ...
    public void validerCommande(User user) {
        try {
            ...
            conn = DataSource.getInstance().getCnx();
            //conn = maConnexion.getInstance().getCnx();
            String query = "UPDATE panier set etat = '" + DataSource.VLIDATED_STATUS + "'  WHERE id_user=" + String.valueOf(user.getIdUser());
            Statement statement = conn.createStatement();
            statement.executeUpdate(query);
        } catch (SQLException ex) {
            Logger.getLogger(Panier.class.getName()).log(Level.SEVERE, null, ex);
    ...
    @Override
    public Map<String, EntreePanier> getCommandeEntries(User user) {
        String productId = "";
        int quantity;
        Produit product = null;
        HashMap<String, EntreePanier> entries = new HashMap<>();
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                ResultSet rs = ps.executeQuery();
                while (rs.next()) {
                    product = Produit.valueOf(rs.getString("name"));
                    EntreePanier entry = new EntreePanier(product, quantity);
    ...
    public List<EntreePanier> getCommandedEntries(User user) {
        String productId = "";
        int quantity;
        Produit product = null;
    ...
    public List<Panier> getPanierbyUser(User user){
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                panier.setDateCreation((Date) date);
                panier.setEtat(etat);
